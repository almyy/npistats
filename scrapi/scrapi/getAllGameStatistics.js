...
const standings = {
    "2014": [ { teamName: 'Muhles Midget Mafia', place: 1 },
    ...
    "2015": [ { teamName: 'Fumble in the Jungle', place: 1 },
    ...
    "2016": [ { teamName: 'White Receivers', place: 1 },
    ...
    "2017": [ { teamName: 'Kentucky Chickens', place: 1 },
...
export const getGamesForCurrentWeek = (response, season, week, standings) => {
    ...
    $('.matchup').each((element, e) => {
        ...
        const teams = $(e).find('.teamWrap').each((i, wrap)=> {
            game["playOff"] = playOffRound > 0;
            if(playOffRound > 0) {
                const teamName = $(wrap).find('.teamName').text();
                if (season <= 2015){
            ...
            $(wrap).find('.userName').each((index, t) => {
